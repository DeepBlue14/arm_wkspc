...
from numpy import array, concatenate, dot, logical_and, ones, zeros
# ROS
import rospy
...
class CloudProxy:
  ...
  def convertToPointCloud2(self, cloud, normals=None):
    '''Convert a point cloud into a ROS PointCloud2 message.'''
    ...
    fields = [PointFieldMsg('x', 0, PointFieldMsg.FLOAT32, 1),
              ...
              PointFieldMsg('normal_x', 12, PointFieldMsg.FLOAT32, 1),
              PointFieldMsg('normal_y', 16, PointFieldMsg.FLOAT32, 1),
              PointFieldMsg('normal_z', 20, PointFieldMsg.FLOAT32, 1)]
    return point_cloud2.create_cloud(header, fields, pts)  
  
  
  def filterNans(self, cloud):
    '''TODO'''
  ...
  def filterWorkspace(self, cloud, workspace):
    '''TODO'''
  ...
  def filterNearAndFarPoints(self, cloud, minDistFromSensor, maxDistFromSensor):
    '''TODO'''
  ...
  def filterNearPoints(self, cloud, distFromSensor):
    '''TODO'''
  ...
  def icp(self, cloud1, cloud2, tol=1e-5, maxIterations=100, doPrint=True):
  ...
  def loadFromPcdFile(self, pointsFileName):
    '''TODO'''
  ...
  def loadFromBinaryPcdFile(self, pointsFileName):
    '''TODO'''
  ...
  def plot(self, cloud, normals=None, nthNormal=0):
  ...
  def removeOutliers(self, cloud, outlierBall=0.025, outlierCount=50):
    '''TODO'''
  ...
  def saveAsMat(self, normals, filename):
    '''
  ...
  def saveAsPcd(self, data, fileName):
    '''
  ...
  def saveAsPcdWithNormals(self, points, normals, fileName):
    '''
    Saves point cloud data to a pcd file.
  ...
  def trimCloudToBall(self, ballCenter, ballRadius, cloud, normals=None):
    '''TODO'''
  ...
  def trimCloudToBallIndices(self, cloud, ballCenter, ballRadius):
    '''TODO'''
  ...
  def updatePlotExtents(self, x, y, z, extents=None):
  ...
  def voxelize(self, cloud, voxelSize=0.003):
    ...
    cloud = numpy.ascontiguousarray(cloud) # cloudprocpy assumes this ordering
    n = cloud.shape[0]
    augment = zeros((n, 1))
    bigCloud = concatenate((cloud, augment), axis=1)    
    cloudTrajopt = cloudprocpy.CloudXYZ()    
