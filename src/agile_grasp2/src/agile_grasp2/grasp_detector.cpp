...
GraspDetector::GraspDetector(ros::NodeHandle& node) : use_incoming_samples_(false), voxel_size_(0.003)
{
  ...
  node.param("remove_outliers", remove_statistical_outliers_, false);
  ...
  learning_  = new Learning(60, params.num_threads_, params.hand_height_);
...
std::vector<GraspHypothesis> GraspDetector::generateHypotheses(const CloudCamera& cloud_cam)
{
  return hand_search_.generateHypotheses(cloud_cam, 0, use_incoming_samples_);
}
...
//  {
//    std::cout << "grasp " << i << ", sample: " << hands_filtered[i].getSample().transpose() << "\n";
//    hands_filtered[i].print();
//  }
...
//    plotter.drawCloud(cloud_cam.getCloudProcessed(), "after");
  ...
  {
    if (num_samples_ > 0 && num_samples_ < indices_.size())
    {
    ...
    else
    {
      ...
      std::cout << "Using all " << indices_.size() << " indices.\n";
    }
  }
  ...
  {
    std::cout << "Calculating surface normals with " << num_threads_ << " CPU threads.\n";
...
void GraspDetector::evaluateGraspPoses(const CloudCamera& cloud_cam, std::vector<GraspHypothesis>& grasps,
  double radius)
{
  hand_search_.evaluateGraspPoses(cloud_cam, grasps, radius);
}
