...
void CloudCamera::subsampleSamples(const std::vector<geometry_msgs::Point>& msg, int num_samples)
{
  // use all incoming samples
  if (num_samples == 0 || num_samples >= msg.size())
  {
    samples_.resize(3, msg.size());
    for (int i = 0; i < samples_.cols(); i++)
    {
      samples_.col(i) << msg[i].x, msg[i].y, msg[i].z;
    }
  ...
  else
  {
    ...
    for (int i = 0; i < samples_.cols(); i++)
    {
      samples_.col(i) << msg[seq[i]].x, msg[seq[i]].y, msg[seq[i]].z;
    }
