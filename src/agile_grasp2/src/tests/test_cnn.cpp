...
//  cloud_cam.subsampleUniformly(num_samples);
 ...
 //    ROS_INFO_STREAM(i << ": " << hands[i].getGraspBottom());
  ...
  Learning learn(60, 1, params.hand_height_);
  std::vector<cv::Mat> image_list = learn.storeLabeledGraspImages(hands, cams_mat, root_dir, true);
...
//  plotter.plotFingers(antipodal_hands, cloud_cam.getCloudOriginal(), "Antipodal Grasps");
  ...
  std::vector<GraspHypothesis> clusters = handle_search.findClusters(antipodal_hands);
  ROS_INFO_STREAM("# handles: " << clusters.size());
  plotter.plotHands(clusters, cloud_cam.getCloudOriginal(), "Clusters");
