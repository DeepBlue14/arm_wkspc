...
const int GraspDetectionNode::CLOUD_SAMPLES = 4; ///< agile_grasp2/CloudIndexed
...
std::vector<GraspHypothesis> GraspDetectionNode::detectGraspPosesInFile(const std::string& file_name_left,
  const std::string& file_name_right, bool use_normals_file)
{
  ...
  if (use_normals_file)
  {
    ...
    cloud_cam->setNormalsFromFile(file_name_right);
  }
  else
  {
...
std::vector<GraspHypothesis> GraspDetectionNode::detectGraspPosesInTopic()
{
  ...
  if (has_normals_)
    cloud_cam = new CloudCamera(cloud_normals_, size_left_cloud_, view_point_);
  // cloud without surface normals
  else
    cloud_cam = new CloudCamera(cloud_, size_left_cloud_, view_point_);
...
bool GraspDetectionNode::graspsServiceCallback(agile_grasp2::FindGrasps::Request& req, agile_grasp2::FindGrasps::Response& resp)
{
  ...
  resp.grasps_msg = createGraspListMsg(hands);
...
void GraspDetectionNode::cloud_sized_callback(const agile_grasp2::CloudSized& msg)
{
  if (!has_cloud_)
  {
    ...
    {
      setNormalsCloudFromMsg(msg.cloud);
    }
    else
    {
      setCloudFromMsg(msg.cloud);
    }

    tf::pointMsgToEigen(msg.view_point, view_point_);
    size_left_cloud_ = msg.size_left.data;
    has_cloud_ = true;
    ROS_INFO_STREAM(size_left_cloud_ << " points from the left and " << cloud_->points.size() - size_left_cloud_
      << " points from the right camera.");
  }
}
...
void GraspDetectionNode::cloud_indexed_callback(const agile_grasp2::CloudIndexed& msg)
{
  if (!has_cloud_)
  {
    ...
    has_cloud_ = true;
    ROS_INFO_STREAM(" Received " << msg.indices.size() << " indices");
  }
}


void GraspDetectionNode::cloud_samples_callback(const agile_grasp2::CloudSamples& msg)
{
  if (!has_cloud_)
  {
    if (msg.cloud.fields.size() == 6 && msg.cloud.fields[3].name == "normal_x" && msg.cloud.fields[4].name == "normal_y"
    ...
    tf::pointMsgToEigen(msg.view_point, view_point_);
    grasp_detector_->setSamplesFromMsg(msg.samples);
...
void GraspDetectionNode::setCloudFromMsg(const sensor_msgs::PointCloud2& msg)
{
...
void GraspDetectionNode::setNormalsCloudFromMsg(const sensor_msgs::PointCloud2& msg)
{
